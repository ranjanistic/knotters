...
def moderationAssignedAlert(moderation: Moderation) -> str:
    ...
    """
    if DeviceNotificationSubscriber.objects.filter(user=moderation.profile.user, device_notification__notification__code=NotificationCode.MODERATION_ASSIGNED).exists():
        user_device_notify(moderation.profile.user, "New Moderation Assigned",
    ...
    if EmailNotificationSubscriber.objects.filter(user=moderation.profile.user, email_notification__notification__code=NotificationCode.MODERATION_ASSIGNED).exists():
        sendActionEmail(
...
def moderationActionAlert(moderation: Moderation, status: str) -> str:
    ...
    """
    if DeviceNotificationSubscriber.objects.filter(user=moderation.profile.user, device_notification__notification__code=NotificationCode.MODERATION_ACTION).exists():
        user_device_notify(moderation.profile.user, f"Moderation {status.capitalize()}",
    ...
    if EmailNotificationSubscriber.objects.filter(user=moderation.profile.user, email_notification__notification__code=NotificationCode.MODERATION_ACTION).exists():
        sendAlertEmail(
            to=moderation.moderator.getEmail(),
