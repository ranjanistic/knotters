...
def managementInvitationSent(invite: ManagementInvitation) -> str:
    ...
    if invite.expired:
        return False
    if DeviceNotificationSubscriber.objects.filter(user=invite.receiver.user, device_notification__notification__code=NotificationCode.MANAGEMENT_INVITATION).exists():
    ...
    if EmailNotificationSubscriber.objects.filter(user=invite.receiver.user, email_notification__notification__code=NotificationCode.MANAGEMENT_INVITATION).exists():
        sendActionEmail(
...
def managementPersonRemoved(mgm: Management, person: Profile) -> str:
    ...
    Returns:
        str: Task ID of the email
    """
    if DeviceNotificationSubscriber.objects.filter(user=person.user, device_notification__notification__code=NotificationCode.MANAGEMENT_PERSON_REMOVED).exists():
    ...
    if EmailNotificationSubscriber.objects.filter(user=person.user, email_notification__notification__code=NotificationCode.MANAGEMENT_PERSON_REMOVED).exists():
        sendAlertEmail(
            to=person.getEmail(),
