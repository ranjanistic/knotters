    ...
    if (iscreator || ismoderator) {
        getElement("uploadprojectimage").onchange = (e) => {
            handleCropImageUpload(
                ...
                "projectimageoutput",
                (croppedB64) => {
                    getElement("uploadprojectimagelabel").innerHTML =
                        "Selected";
    ...
    if (iscreator || ismoderator || iscocreator) {
        ...
        getElements("visibility-assets-action").forEach(async (action) => {
            action.onclick = async () => {
                ...
                if (data.code === code.OK) {
                    message(
                        `Asset is ${makepublic ? "public" : "private"} now.`
                    );
                    ...
                    action.title = makepublic
                        ? `${APPNAME} community can access`
                        : "Only you can access";
                    return true;
                }
                error(data.error);
        ...
        getElement("save-edit-projecttags").onclick = async () => {
            ...
            var resp = await postRequest(
                setUrlParams(URLS.TAGSUPDATE, projectID),
                {
            ...
            cocreator.onclick = async (e) => {
                ...
                if (data.code === code.OK) {
                    ...
                    if (getElements("cancel-cocreator-invite").length > 1) {
                        getElement(
                            `cocreator-view-${cocreator.getAttribute(
                                "data-userid"
                            )}`
                        ).remove();
                        return true;
    ...
    getElement("show-admirations").onclick = async (_) => {
        Swal.fire({
            html: await getRequest2({
                path: setUrlParams(URLS.ADMIRATIONS, projectID),
            }),
    ...
    const loadLiveData = async () => {
        ...
        if (contribview || languageview || commitsview) {
            const data = await getRequest(
                setUrlParams(URLS.LIVEDATA, projectID)
            );
            if (!data || data.code !== code.OK) {
                if (languageview) {
                    setHtmlContent(
                    ...
                    getElement(`livelangdataretry`).onclick = (_) =>
                        loadLiveData();
                }
                if (commitsview) {
                    setHtmlContent(
                ...
                if (contribview) {
                    setHtmlContent(
                        contribview,
                        loadErrorHTML(`livecontdataretry`)
                    );
                    getElement(`livecontdataretry`).onclick = (_) =>
                        loadLiveData();
                }
                return;
            }
            ...
            if (commitsview) {
                if (data.commitsHTML)
                    setHtmlContent(commitsview, data.commitsHTML);
