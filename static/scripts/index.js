...
const newUpdateBar = () => {
  alertify
    .confirm(
      ...
      `A new version of Knotters is available. Please update the app by clicking on 'Update'.`,
      () => {
        alertify.success("Updating...");
        newServiceWorker.postMessage({ action: "skipWaiting" });
      },
      () => {
        alertify.error("Try updating later to avail new features! Thank you.");
      }
    )
    .set("labels", { ok: "Update", cancel: "Not now" }, "modal", true);
};
...
const loader = (show = true) => {
  getElement("viewloader").innerHTML = show ? `<div class="loader"></div>` : "";
