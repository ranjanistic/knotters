...
class SSHKey(models.Model):
    """The model for storing SSH key"""
    id: UUID = models.UUIDField(
        primary_key=True, default=uuid4, editable=False)
    public_key: str = models.CharField(max_length=10000, unique=True,
                                help_text="SSH public key")
    """public_key (CharField): SSH public key."""
    name: str = models.CharField(max_length=100)
    """name (CharField): name of the SSH key"""
    creator = models.ForeignKey(
        f'{PEOPLE}.Profile', related_name='sshkey_creator_profile', on_delete=models.CASCADE)
    """creator (ForeignKey<Profile>): user of this SSH key"""
    created_on: datetime = models.DateTimeField(
        auto_now=False, default=timezone.now)
    """created_on (DateTimeField): date and time when this SSH key was added"""
