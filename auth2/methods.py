...
def migrateUserAssets(predecessor: User, successor: User) -> bool:
    try:
        ...
        CoreProject.objects.filter(
            creator=predecessor.profile, status=Code.MODERATION).delete()
        ...
        Project.objects.filter(creator=predecessor.profile, trashed=False).update(
            migrated=True, migrated_by=predecessor.profile, migrated_on=timezone.now(), creator=successor.profile)
        ...
        if len(cprojs):
            if successor.profile.is_manager():
                cprojs.update(migrated=True, migrated_by=predecessor.profile,
                              migrated_on=timezone.now(), creator=successor.profile)
            else:
                raise Exception(
                    "Cannot migrate core projects, successor is not manager.", predecessor, successor)
        return True
