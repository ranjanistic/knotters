...
def increaseXPInTopicAlert(profile: Profile, incrementPoints: int, topicName: str, topicPoints: int):
    """
    Send device notification to user when their Xp is decreased in a topic 
    """
    if DeviceNotificationSubscriber.objects.filter(user=profile.user, device_notification__notification__code=NotificationCode.INCREASE_XP_IN_TOPIC).exists():
        user_device_notify(profile.user, "Topic XP increased.",
                           f"You have gained {incrementPoints} XP in {topicName}. {topicPoints} is your current XP.", profile.get_abs_link)


def decreaseXPInTopicAlert(profile: Profile, decrementPoints: int, topicName: str, topicPoints: int):
    """
    Send device notification to user when their Xp is decreased in a topic 
    """
    if DeviceNotificationSubscriber.objects.filter(user=profile.user, device_notification__notification__code=NotificationCode.DECREASE_XP_IN_TOPIC).exists():
        user_device_notify(profile.user, "Topic XP decreased.",
                           f"You have lost -{decrementPoints} XP in {topicName}. {topicPoints} is your current XP.", profile.get_abs_link)
